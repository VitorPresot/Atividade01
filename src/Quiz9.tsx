...
export default function Quiz9() {
  ...
  const question = "Qual é o principal benefício de usar TypeScript?";
  const options = [
    { id: '1', text: 'Tipagem estática para maior segurança', correct: true },
    { id: '2', text: 'Melhor performance em tempo de execução', correct: false },
    { id: '3', text: 'Reduzir o tamanho do arquivo', correct: false },
    { id: '4', text: 'Simplificar a sintaxe', correct: false },
  ];
  const checkAnswer = () => {
    const selectedOption = options.find(opt => opt.id === selectedAnswer);
    const isCorrect = selectedOption?.correct || false;
    const newPoints = isCorrect ? pontos + 10 : pontos;
    router.push({ pathname: '/Quiz10', params: { pontos: newPoints } });
  };
  const progressPercent = (9 / 10) * 100;
  return (
      ...
      <View style={[styles.container, { paddingTop: 30, paddingBottom: 30 }]}>
        <View style={styles.hudContainer}>
          <View style={styles.hudItem}>
            ...
            <Text style={styles.hudValue}>9/10</Text>
          </View>
          <View style={styles.hudItem}>
        ...
        <View style={styles.questionContainer}>
          <Text style={styles.questionNumber}>⚔️ DESAFIO 9</Text>

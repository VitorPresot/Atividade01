...
export default function Quiz6() {
  ...
  const question = "O que é uma função assíncrona?";
  const options = [
    { id: '1', text: 'Função que executa em paralelo com outras operações', correct: true },
    { id: '2', text: 'Função que sempre retorna nulo', correct: false },
    { id: '3', text: 'Função que só funciona em aplicativos', correct: false },
    { id: '4', text: 'Função que executa imediatamente', correct: false },
  ];
  const checkAnswer = () => {
    const selectedOption = options.find(opt => opt.id === selectedAnswer);
    const isCorrect = selectedOption?.correct || false;
    const newPoints = isCorrect ? pontos + 10 : pontos;
    router.push({ pathname: '/Quiz7', params: { pontos: newPoints } });
  };
  const progressPercent = (6 / 10) * 100;
  return (
      ...
      <View style={[styles.container, { paddingTop: 30, paddingBottom: 30 }]}>
        <View style={styles.hudContainer}>
          <View style={styles.hudItem}>
            ...
            <Text style={styles.hudValue}>6/10</Text>
          </View>
          <View style={styles.hudItem}>
        ...
        <View style={styles.questionContainer}>
          <Text style={styles.questionNumber}>⚔️ DESAFIO 6</Text>

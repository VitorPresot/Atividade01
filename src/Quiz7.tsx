...
export default function Quiz7() {
  ...
  const question = "Qual framework estamos usando neste projeto?";
  const options = [
    { id: '1', text: 'Flutter', correct: false },
    { id: '2', text: 'React Native com Expo', correct: true },
    { id: '3', text: 'Swift', correct: false },
    { id: '4', text: 'Kotlin', correct: false },
  ];
  const checkAnswer = () => {
    const selectedOption = options.find(opt => opt.id === selectedAnswer);
    const isCorrect = selectedOption?.correct || false;
    const newPoints = isCorrect ? pontos + 10 : pontos;
    router.push({ pathname: '/Quiz8', params: { pontos: newPoints } });
  };
  const progressPercent = (7 / 10) * 100;
  return (
      ...
      <View style={[styles.container, { paddingTop: 30, paddingBottom: 30 }]}>
        <View style={styles.hudContainer}>
          <View style={styles.hudItem}>
            ...
            <Text style={styles.hudValue}>7/10</Text>
          </View>
          <View style={styles.hudItem}>
        ...
        <View style={styles.questionContainer}>
          <Text style={styles.questionNumber}>⚔️ DESAFIO 7</Text>
